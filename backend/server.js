...
app.get("/:account_id/comments", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "SELECT comment.*, post.title,post.description,account.username FROM post JOIN comment on post.post_id = comment.post_id JOIN account ON account.account_id = comment.account_id WHERE comment.account_id = $1 ORDER BY date DESC;",
      [account_id]
    );
    res.status(200).json(result.rows);
  } catch (error) {
